...
const { boilerplate_AWS_DB_SECRET_ARN } = process.env;
...
export const aggregateRows = (data, identifier, merge = basicMerge) => {
  ...
  return Object.entries(groupBy).map(([key, items]) => merge(items));
};

export default async function db() {
  const secret = await getSecretValue(boilerplate_AWS_DB_SECRET_ARN!);
